...
func GetSessionByID(id int, user_id uuid.UUID) (*models.Toast, error) {
	query := `SELECT session_name, description, start_time, end_time, duration, active, category_id FROM toast WHERE id = $1 AND user_id = $2`
	var s models.Toast
	s.UserID = user_id
	s.SessionID = id
	var endTime sql.NullTime
	err := R.Instance.QueryRow(query, id, user_id).Scan(&s.SessionName, &s.Description, &s.StartTime, &endTime, &s.Duration, &s.Active, &s.CategoryID)
	if err != nil {
